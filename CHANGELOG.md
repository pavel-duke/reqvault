...
- command palette по `Ctrl+K` / `Cmd+K` с поиском запросов и основными действиями;
- вкладки запросов с переключением, признаком изменений и восстановлением после перезапуска;
...
- массовое перемещение, переименование и дублирование запросов без перезаписи файлов;
- виртуализация дерева больших workspace;
- Dependabot, CodeQL, gitleaks, RustSec, SPDX SBOM, SHA-256 и provenance attestations для релизов.

### Изменено

- Production Guard получил отдельные правила для private, loopback, link-local и cloud metadata адресов;
- redirect выполняется вручную с проверкой каждого нового URL и ограничением длины цепочки;
- повторный запуск desktop-приложения возвращает фокус существующему окну вместо открытия второго экземпляра.

### Исправлено

- чувствительные заголовки больше не переносятся при смене схемы, hostname или порта;
- AWS Signature V4 не следует на другой origin с подписью исходного запроса;
- массовые файловые операции проверяют все пути до записи и откатывают уже выполненное перемещение при ошибке;
- восстановленный черновик не сохраняет значения credential и тело ответа.

### Безопасность

- добавлены SSRF-проверки DNS, локальных сетей и metadata endpoints с явным allowlist;
- временные структуры с токенами, паролями, request body и PEM identity очищаются из памяти там, где это поддерживается;
- mTLS предупреждает о незашифрованном PEM-ключе без показа ключа или пути в ошибке;
- секреты не индексируются, не сохраняются во вкладках и не попадают в историю или диагностические сообщения;
- security workflow блокирует релиз при уязвимостях, утечках секретов и ошибках анализа.
