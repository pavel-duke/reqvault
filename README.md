...
> ReqVault 1.1 — стабильная версия для работы с большими API-проектами. Приложение работает локально, не требует регистрации и сохраняет совместимость с форматом workspace v1.
...
- command palette по `Ctrl+K` / `Cmd+K` с поиском и быстрыми действиями;
- вкладки запросов с безопасным восстановлением несохранённых черновиков;
- закреплённые и недавно открытые запросы, которые хранятся локально;
...
- защита от SSRF и переноса credential при redirect на другой origin.
...
Следующий этап — сценарии и цепочки запросов в версии 1.2.0. Затем появятся contract testing, инструменты работы через Git и нагрузочные прогоны.
...
- Проверка безопасности 1.1 опубликована в [`docs/security-review-1.1.md`](docs/security-review-1.1.md).
- Проверки цепочки поставки описаны в [`docs/supply-chain.md`](docs/supply-chain.md).
